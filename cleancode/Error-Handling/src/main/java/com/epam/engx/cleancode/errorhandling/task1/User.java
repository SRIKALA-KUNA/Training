...
public class User {
    ...
    public Address getPreferredAddress() {
        try {
            ...
            if (isNotDeliveryAddresses(deliveryAddresses))
                return deliveryAddresses.get(0);
            else if (isNotOrderAddresses(orderAddresses))
	...
	public boolean isNotOrderAddresses(List<Address> orderAddresses) {
		return !orderAddresses.isEmpty();
	}

    private boolean isNotDeliveryAddresses(List<Address> deliveryAddresses) {
