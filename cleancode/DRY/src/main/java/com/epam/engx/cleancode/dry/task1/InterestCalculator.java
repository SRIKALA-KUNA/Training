...
public class InterestCalculator {
    ...
    private int durationBetweenDatesInYears(Date from, Date to) {
        Calendar startCalendar = startCalender(from);
        Calendar endCalendar = endCalender(to);

        return diffYear(startCalendar, endCalendar);
    }

	public int diffYear(Calendar startCalendar, Calendar endCalendar) {
	...
	public Calendar endCalender(Date to) {
		Calendar endCalendar = new GregorianCalendar();
        endCalendar.setTime(to);
		return endCalendar;
	}

	public Calendar startCalender(Date from) {
    ...
    private BigDecimal interest(AccountDetails accountDetails) {
        ...
        if (isAccountStartedAfterBonusAge(accountDetails)) {
            if (AGE <= accountDetails.getAge()) {
                ...
                interest = interestCalculation(accountDetails,SENIOR_PERCENT );
            } else {
                interest =interestCalculation(accountDetails,INTEREST_PERCENT );
            }
        }
        return BigDecimal.valueOf(interest);
    }

	public double interestCalculation(AccountDetails accountDetails, double percent) {
		double interest;
    ...
    private int durationSinceStartDateInYears(Date startDate) {
        Calendar startCalendar = startCalender(startDate);
        Calendar endCalendar = new GregorianCalendar();
